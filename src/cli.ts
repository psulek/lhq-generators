...
import { detectLineEndings, serializeLhqModelToString, serializeRootElement } from './generatorUtils';
...
async function saveLhqModelFile(model: LhqModel, fileName: string, fileContent?: string): Promise<void> {
    ...
    const json = serializeLhqModelToString(model, lineEndings);
...
async function generateFromLhq(hbsTemplatesDir: string, lhqFileName: string, csProjectFileName: string, outputDir: string,
    ...
    if (fixNamespace) {
        ...
        //             model = serializeRootElement(rootModel);
...
async function findNamespaceFromCsProj(csProjectFileName: string, lhqFileName: string): Promise<string> {
    ...
    if (isNullOrEmpty(rootNamespace)) {
        ...
        rootNamespace = generatorUtils.getRootNamespaceFromCsProj(path.basename(lhqFileName), t4FileName, csProjectFileName, csProjectFileContent)?.namespace ?? '';

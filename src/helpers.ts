...
import type { IHostEnvironment, KeysMatching, TextEncodeModes } from './types';
...
function setCustomData(context: unknown, options: HbsDataContext, valueOrFn: () => unknown, forceToRoot: boolean): void {
    const value = typeof valueOrFn === 'function' ? valueOrFn() : valueOrFn;
    const key = valueOrDefault<string>(options?.hash?.key, '');

import type { CodeGeneratorGroupSettings, ICodeGeneratorSettingsConvertor } from './api/modelTypes';
            ...
            node.childs.forEach(child => {
                ...
                if (!isNullOrEmpty(groupName) && child.attrs !== undefined) {
                    if (Object.prototype.hasOwnProperty.call(definition.settings, groupName)) {
                        ...
                        //     const property = groupSettings.find(x => x.name === name);
                        //     if (property) {
                        //         settings[name] = convertValue(value, property);
                        //     }
                        // }
